...
#------------------------------------------------------------   
def convertMatrixToDictionary(tickets_list): #O(N) where N is length of list
...
def bubleSort(tickets_list):#Using Bubble Sort O(N*N)(take the max O)
  ...
  for x in range(l):#O(N) where N is the lenght of list
    ...
    for y in range(l-x-1):#O(N)
      ...
      if tickets_list[y][3] == tickets_list[y+1][3]:#here if same date
...
#-------------------------------------------------- 
def displayAllTickets(tickets_list,current_date): #O(N) where N is the len of list
  new_list = [] #here to only the events' info from the current date and append them in a new list
  tickets_list = bubleSort(tickets_list)  
  for i in range(len(tickets_list)):
    if tickets_list[i][3] >= current_date:
      new_list.append(tickets_list[i])
  return new_list

#--------------------------------------------------
def changePriority(tickets_list):
...
#--------------------------------------------------
def runEvent(tickets_list, current_date): #O(N) (take the worst case)
  todayEvent = []
  for i in range(len(tickets_list)):#O(N) n is the length of tickets_list
    if tickets_list[i][3] == current_date:
      todayEvent.append(tickets_list[i])
  todayEvent = sorted(todayEvent, key=lambda x: x[4]) #O(NlogN)
...
def adminMenu(tickets_list):
  ...
  while not exit_admin_menu:
    ...
    if choice == "1":
      ...
      current_date = input("Enter the current Date (YYYYNNDD): ")
      todayEvent = runEvent(tickets_list, current_date)
      print("Today's Event you want to remove:\n", todayEvent)
      for i in range(len(todayEvent)):#Remove today's events
        del todayEvent[0:i]
    elif choice == "7":
      print("\n------>You Exited!")
